...
export class WeatherCalculator {
  ...
  static calculateWeatherChanges(params) {
    ...
    const baseValues = this._calculateBaseValues(
      terrain,
      savedState,
      variability
    );
    ...
    // Get time period
    const timePeriod = TimeUtils.getTimePeriod();
    ...
    const timeModifiers = {
      temperature:
        (globalTimeModifiers.temperature || 0) +
        (terrainTimeModifiers.temperature || 0),
      wind: (globalTimeModifiers.wind || 0) + (terrainTimeModifiers.wind || 0),
      precipitation:
        (globalTimeModifiers.precipitation || 0) +
        (terrainTimeModifiers.precipitation || 0),
      humidity:
        (globalTimeModifiers.humidity || 0) +
        (terrainTimeModifiers.humidity || 0),
    };
    ...
    const seasonModifiers = this._getSeasonModifiers(
      currentSeason,
      settingsData
    );
  ...
  static _applyModifiers(baseValues, timeModifiers, seasonModifiers) {
    ...
    const finalWind =
      baseValues.wind + (timeModifiers.wind || 0) + (seasonModifiers.wind || 0);
    ...
    for (let i = 0; i < days; i++) {
      ...
      const dayWeather = {
        ...
        wind: Math.max(
          ...
          Math.min(
            10,
            Math.round((terrain.wind + previousWeather.wind) / 2 + randWind)
          )
        ),
        precipitation: Math.max(
          ...
          Math.min(
            10,
            Math.round(
              (terrain.precipitation + previousWeather.precipitation) / 2 +
                randPrecip
            )
          )
        ),
